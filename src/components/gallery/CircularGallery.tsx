...
const DEFAULT_FONT = 'bold 30px "Noto Serif SC", "Playfair Display", serif';
const DEFAULT_FONT_URL = 'https://fonts.googleapis.com/css2?family=Noto+Serif+SC:wght@600;700&family=Playfair+Display:ital,wght@1,600&display=swap';
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          // Smooth antialiasing for borderless luxury aesthetic
...
class App {
  constructor(
    container: HTMLElement,
    {
      ...
      textColor = '#ffffff',
      borderRadius = 0.03,
      font = 'bold 30px "Noto Serif SC", "Playfair Display", serif',
      scrollSpeed = 2,
      scrollEase = 0.05,
      onActiveIndexChange
    }: any = {}
  ...
  createMedias(items: any[], bend = 2.5, textColor: string, borderRadius: number, font: string) {
    const defaultItems = [
      { image: `https://images.unsplash.com/photo-1555939594-58d7cb561ad1?auto=format&fit=crop&q=80&w=800`, text: 'Socrates' },
      { image: `https://images.unsplash.com/photo-1579783902614-a3fb3927b675?auto=format&fit=crop&q=80&w=800`, text: 'Plato' }
    ];
  ...
  onTouchDown(e: any) {
    // Only capture touch/mouse if directly initiated on the gallery canvas
  ...
  onCheck() {
    ...
    if (this.onActiveIndexChange && this.galleryItems && this.galleryItems.length) {
      let activeIdx = Math.round(this.scroll.target / width) % this.galleryItems.length;
      if (activeIdx < 0) activeIdx = (activeIdx + this.galleryItems.length) % this.galleryItems.length;
  ...
  addEventListeners() {
    ...
    // Scoped strictly to gallery container to prevent global window click interference
    if (this.container) {
...
export default function CircularGallery({
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="3D WebGL Circular Philosophy Gallery. Scroll or drag to explore."
    />
  );
}
